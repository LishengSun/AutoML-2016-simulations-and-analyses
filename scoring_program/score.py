"""
Modified by: Lisheng Sun, AutoML 2016 analyses July 2017

Before any run, change if necessary:
    - root_dir
    - default_input_dir
    - default_output_dir
    - line 94: predict files -> point to where you store prediction files for scoring
"""
#!/usr/bin/env python
...
default_input_dir = root_dir + "simulation_results/" 
default_output_dir = root_dir + "scores_of_simulation_results/winner_abhishek/"#_%s/"%the_date  
...
if __name__=="__main__":
    ...
    for solution_file in solution_names:
        ...
        try:
            ...
            if (predict_files == []): raise IOError('Missing prediction file {}'.format(basename))
            ...
            # Read the solution and prediction values into numpy arrays
                ...
                if same_loss:
                    try:
                        if info['task'] == 'regression': # use r2 loss for regression
                            score_name = 'r2_metric'
                            solution = sanitize_array (solution); prediction = sanitize_array (prediction)  
                            score = eval('r2_metric' + '(solution, prediction, "' + info['task'] + '")')
                        else: # use bac for classification
                            score_name = 'bac_metric'
                        ...
                        # score_file.write("set%d" % set_num + " (" + predict_name.capitalize() + "): score(" + score_name + ")=%0.12f =======\n" % score)
